...
private:
    ...
    void InitializeButtons() {
        ...
        key_button_.OnLongPress([this]() {
            ...
            if (led && !key_long_pressed && (now - last_key_press_time) > LONG_PRESS_TIMEOUT_US) {
                key_long_pressed = true;
                led->SetBrightness(0);
                shutdown_ready_ = true;
                ESP_LOGI(TAG, "Long press detected, ready for shutdown");
            }
        });
    }
    ...
    void BlinkGreenFor5s() {
        ...
        esp_timer_create_args_t timer_args = {
            .callback = [](void* arg) {
                auto* self = static_cast<XiaoHeShangS3Board*>(arg);
...
public:
    XiaoHeShangS3Board() : boot_button_(BOOT_BUTTON_GPIO), key_button_(KEY_BUTTON_GPIO, true) {
        InitializePowerCtl();
        InitializeADC();
        InitializeI2c();
        InitializeButtons();
...
DECLARE_BOARD(XiaoHeShangS3Board);
